...
// .schema("AT")
// .from("tesis")
// .select("id, estudiante_id, titulo, carpeta_drive_id")
// .eq("id", tesisId)
...
// return new Response(
// JSON.stringify({
// error: "La tesis no tiene carpeta de Drive asignada",
// }),
// {
// status: 400,
// headers: { ...corsHeaders, "Content-Type": "application/json" },
...
// let nextVersion = 1;
//
// folderId = tesis.carpeta_drive_id as string;
//
// if (!folderId) {
// return new Response(
// JSON.stringify({
// error: "No se pudo determinar carpeta Drive",
// }),
// {
// status: 500,
// headers: { ...corsHeaders, "Content-Type": "application/json" },
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (usuarioATError || !usuarioAT) {
      return new Response(
        JSON.stringify({
          error: "Usuario AT inválido",
          debug: {
            auth_user_id: user.id,
            usuarioAT,
            usuarioATError,
    ...
    let tesis: TesisRow | null = null;

    if (usuarioAT.rol === "estudiante") {
      ...
      tesis = data;
    } else if (usuarioAT.rol === "asesor") {
      const { data, error } = await supabaseAdmin
        .schema("AT")
        .from("asesores_tesis")
        .select("tesis:tesis_id(id, estudiante_id, titulo, carpeta_drive_id)")
        .eq("tesis_id", tesisId)
        .eq("asesor_id", usuarioAT.id)
        .eq("activo", true)
        .maybeSingle();
      ...
      if (error) {
        return new Response(
          JSON.stringify({
            error: "Error consultando tesis asignada al asesor",
            details: error.message,
          }),
          {
            status: 500,
      ...
      tesis = (data?.tesis as TesisRow | undefined) ?? null;
    } else {
      return new Response(
        JSON.stringify({
          error: "Rol no permitido para subir documentos",
          rol: usuarioAT.rol,
        }),
        {
          status: 403,
    ...
    const folderId = tesis.carpeta_drive_id;

    let nextVersion = 1;
    if (modo === "tesis") {
      const { data: ultimoDoc, error: ultimoDocError } = await supabaseAdmin
        .schema("AT")
        .from("documentos_tesis")
        .select("version")
        .eq("tesis_id", tesisId)
      ...
      if (ultimoDocError) {
        return new Response(
          JSON.stringify({
            error: "Error consultando última versión",
            details: ultimoDocError.message,
          }),
          {
            status: 500,
            headers: { ...corsHeaders, "Content-Type": "application/json" },
