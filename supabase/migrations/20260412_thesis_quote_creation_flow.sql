...
add column if not exists "tipo_tesis_id" uuid references "AT"."tipos_tesis"("id"),
add column if not exists "plan_id" uuid references "AT"."planes"("id"),
add column if not exists "programa_id" uuid references "AT"."programas"("id"),
add column if not exists "nivel_academico" varchar,
add column if not exists "requiere_analisis_estadistico" boolean default true,
add column if not exists "precio_base" numeric(10,2),
add column if not exists "porcentaje_nivel" numeric(5,2),
add column if not exists "monto_ajuste_nivel" numeric(10,2) default 0,
add column if not exists "descuento_analisis_estadistico" numeric(10,2) default 0,
add column if not exists "precio_total" numeric(10,2),
add column if not exists "moneda" varchar default 'PEN';

alter table "AT"."pagos"
add column if not exists "tesis_id" uuid references "AT"."tesis"("id");
...
begin
  for v_constraint in
    ...
    where n.nspname = 'AT'
      ...
      and pg_get_constraintdef(c.oid) ilike '%borrador%'
  loop
    execute format(
      'alter table "AT"."tesis" drop constraint if exists %I',
  ...
  if not exists (
    ...
    where n.nspname = 'AT'
      and t.relname = 'tesis'
      and c.conname = 'tesis_estado_check'
  ) then
    alter table "AT"."tesis"
...
create or replace function "AT"."obtener_tipos_tesis_activos"()
returns table (
  "id" uuid,
  "codigo" varchar,
  "nombre" varchar
...
create or replace function "AT"."cotizar_tesis_plan"(
  p_plan_id uuid,
  p_tipo_tesis_id uuid,
...
begin
  ...
  v_monto_ajuste_nivel := round(
    (v_precio_base * v_porcentaje_nivel / 100.0)::numeric,
    2
  );
...
begin
  ...
  if v_estudiante_id is null then
    raise exception 'No se encontró el estudiante autenticado';
  end if;
